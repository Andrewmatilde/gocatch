...
func main() {
	...
	Wl := gocatch.CreatWorkLineList(m, time.Duration(100)*time.Millisecond)
	Al := gocatch.CreateAnaLineList(m, time.Duration(100)*time.Millisecond)
	gocatch.RunManager(m, Wl, Al)
}
